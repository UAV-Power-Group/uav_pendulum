...
/*
    ...
    // 【订阅】tf mini的数据
    ros::Subscriber tfmini_sub = nh.subscribe<sensor_msgs::Range>("/TFmini/tfmini_link", 100, tfmini_cb);
*/
    // 【订阅】optitrack估计位置
    ros::Subscriber optitrack_sub = nh.subscribe<geometry_msgs::PoseStamped>("/vrpn_client_node/UAV_6_3/pose", 1000, optitrack_cb);
    ...
    //ros::Timer,ros中的定时器
    //设置频率为200HZ
    ros::Timer pose_pub_timer = nh.createTimer(ros::Duration(1.0/100.0),pose_pub_timer_cb);
